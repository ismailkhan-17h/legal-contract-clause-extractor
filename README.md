...
1. Paste contract text, or upload a contract as a PDF
2. Run clause extraction
3. Receive plain, labeled output for each clause (governing law, termination, indemnification, liability cap, non-compete, exclusivity)

The extractor is hybrid: it runs the fine-tuned model, and for any clause the
model gets wrong or fails to return, a deterministic rule-based extractor
fills in the gap. This keeps results reliable even though the base model is
small (SmolLM2-360M) and trained on a limited number of examples.
...
Non-Compete: Twelve months after termination
```
